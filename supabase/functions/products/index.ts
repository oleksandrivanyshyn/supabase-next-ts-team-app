...
function mapProductRow(
  ...
  return {
    ...
    createdBy: {
      ...
      displayName: product.profiles!.display_name,
    },
...
async function toProductDto(
  ...
  if (product.image_path) {
    ...
    if (storageError)
      console.error('Failed to sign product image URL:', storageError.message);
  ...
  try {
    ...
    if (req.method === 'GET' && segments.length === 1) {
      ...
      const pageSize = Math.min(
        100,
        Math.max(1, Number.isNaN(parsedPageSize) ? 20 : parsedPageSize),
      );
      ...
      const { status, dateFrom, dateTo, createdBy, search } = validate(
        queryParams,
        productListQuerySchema,
      );
      ...
      if (imagePaths.length > 0) {
        ...
        if (storageError)
          console.error(
            'Failed to sign product image URLs:',
            storageError.message,
          );

        if (!storageError && signedData) {
          signedUrlsMap = signedData.reduce(
            (acc, current) => {
      ...
      const responseData =
        products?.map((p) =>
          mapProductRow(
            p,
            p.image_path ? signedUrlsMap[p.image_path] || null : null,
          ),
        ) || [];
    ...
    if (req.method === 'GET' && segments.length === 2) {
      ...
      return jsonResponse({
        product: await toProductDto(ctx.supabase, product),
      });
    }
    ...
    if (req.method === 'PATCH' && segments.length === 2) {
      ...
      if (
        ...
        (result.title !== undefined ||
          ...
          result.imagePath !== undefined)
      ) {
        throw new HttpError(
          409,
          'Active or deleted products cannot be edited, only their status can change',
        );
      }
      ...
      if (result.imagePath !== undefined)
        updateData.image_path = result.imagePath || null;
      ...
      return jsonResponse({
        product: await toProductDto(ctx.supabase, updatedProduct),
      });
    }
